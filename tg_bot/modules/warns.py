...
def set_warn_strength(bot: Bot, update: Update, args: List[str]):
    ...
    if args:
        if args[0].lower() in ("on", "yes"):
            ...
            msg.reply_text("Too many warns will now result in a forbid!")
            return "<b>{}:</b>\n" \
                   "<b>Admin:</b> {}\n" \
                   "Has enabled strong warns. Users will be forbidden/banned.".format(html.escape(chat.title),
                                                                            mention_html(user.id, user.first_name))
            ...
            return "<b>{}:</b>\n" \
                   "<b>Admin:</b> {}\n" \
                   "Has disabled strong warns. Users will only be bitten/kicked.".format(html.escape(chat.title),
        ...
        limit, soft_warn = sql.get_warn_setting(chat.id)
        if soft_warn:
            msg.reply_text("Warns are currently set to *bite* users when they exceed the limits.",
                           parse_mode=ParseMode.MARKDOWN)
        else:
            msg.reply_text("Warns are currently set to *forbid* users when they exceed the limits.",
                           parse_mode=ParseMode.MARKDOWN)
    return ""
...
def __chat_settings__(chat_id, user_id):
    ...
    return "This chat has `{}` warn filters. It takes `{}` warns " \
           "before the user gets *{}*.".format(num_warn_filters, limit, "bitten" if soft_warn else "forbidden")
